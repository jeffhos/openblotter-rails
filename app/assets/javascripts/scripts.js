...
$(window).load(function() {
	...
	// Pull data
  var url = "/incidents?startDate=" + startDate + "&endDate=" + endDate + "&startTime=" +
	...
	$.getJSON(url, function(data) {
		...
		$.each(data, function(i, incident) {
			...
			// Add timestamp
			newIncident.append("<div class = 'timestamp'>" + incident["time"] + " &mdash; " +
        incident["type"] + " </div>");
			...
			info.append("<table><tr><td><i class='fa fa-map-marker'></td><td><h2>" +
        incident.neighborhood + "</h2><h2>" + incident.address + "</h2></td>");
		...
		// Add aggregated charges
    url = "/aggregates?startDate=" + startDate + "&endDate=" + endDate + "&startTime=" +
      startTime + "&endTime=" + endTime;
		$.getJSON(url, function(data){
...
function resize() {
  ...
  var content = $("#content");
	$("head").append("<style type='text/css'>#content:after{ height: " + content.height() +
    "px }</style>");
	content.css("height", "100%");

  // Sorry about using 40px; it's arbitrary but works
	content.css("height", $(window).height() - $("#header").height() - 40);
}
